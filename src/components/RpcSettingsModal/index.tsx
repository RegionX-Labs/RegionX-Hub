...
const RpcSettingsModal: React.FC<RpcSettingsModalProps> = ({ isOpen, onClose, onRpcChange }) => {
  ...
  useEffect(() => {
    ...
    const assetHubKey = `${relayKey}AH` as keyof typeof chains;
    ...
    const assetHubRpcs = assetHubChain?.nodes || relayChain?.nodes || [];
    ...
    if (storedRaw) {
      try {
        ...
        const networkSettings = parsed?.[network];
        storedAssetHub = networkSettings?.assetHubUrl || networkSettings?.relayUrl || '';
        storedCoretime = networkSettings?.coretimeUrl || '';
      } catch {
        storedAssetHub = '';
        storedCoretime = '';
      }
